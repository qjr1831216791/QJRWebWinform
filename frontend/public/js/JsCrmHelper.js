...
import { rtcrm } from './rtcrm.min.js';

if (typeof JsCrm !== 'object') {
    var JsCrm = {};
}
...
// 支持从全局变量获取（避免循环依赖问题）
function getContext() {
    // 优先从全局变量获取（main.js 中设置的）
    if (typeof window !== 'undefined' && window.__VUE_CONTEXT__) {
        return window.__VUE_CONTEXT__;
    }

    // 如果全局变量不存在，尝试从模块导入（可能因为循环依赖而失败）
    try {
        // 动态导入避免循环依赖
        if (typeof require !== 'undefined') {
            var mainModule = require('../../src/main.js');
    ...
    // 如果都不存在，返回一个基础对象（避免报错）
    return {
        envconfig: typeof window !== 'undefined' && window.__WebConfig__ ? window.__WebConfig__ : {},
        axios: typeof window !== 'undefined' && window.axios ? window.axios : null,
        $globalVar: typeof window !== 'undefined' && window.__globalVar__ ? window.__globalVar__ : {},
        $confirm: function () { return Promise.resolve(); },
        $loading: function () { return { close: function () { } }; }
    };
}

// 定义 context 变量，使用 getter 动态获取
var context = {
    get envconfig() {
        return getContext().envconfig || {};
    },
    get axios() {
        return getContext().axios || (typeof axios !== 'undefined' ? axios : null);
    },
    get $globalVar() {
        return getContext().$globalVar || {};
    },
    $confirm: function () {
        var ctx = getContext();
        return ctx.$confirm.apply(ctx, arguments);
    },
    $loading: function (options) {
        var ctx = getContext();
        return ctx.$loading.call(ctx, options);
    }
};
...
JsCrm.common = {
    ...
    GetEmptyGuid: function () {
        ///	<summary>获取空Guid</summary>
        return "00000000-0000-0000-0000-000000000000";
    },

    openAlertDialog: function (_this, message, title, onCloseCallback) {
    ...
    openConfirmDialog: function (confirmStrings, confirmOptions, successCallback, errorCallback) {
        context.$confirm(confirmStrings, '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            dangerouslyUseHTMLString: true
        }).then((res) => {
            if (successCallback)
                successCallback({ confirmed: true });
        }).catch((err) => {
            if (errorCallback)
                errorCallback({ confirmed: false });
        });
    },
    ...
    showLoading: function (message) {
        ...
        this.__loading = context.$loading({
            lock: true,
            text: message,
...
JsCrm.webApi = {
    ...
    ApiPost: function (apiName, params, isDataSubmit) {
        ...
        if (rtcrm.isNull(isDataSubmit)) isDataSubmit = true;
        return new Promise((resolve, reject) => {
            context.axios({
                url: apiBaseUri + apiName,
                method: 'POST',
                params: isDataSubmit ? null : params,
                data: isDataSubmit ? params : null,
                headers: customHeaders,
            }).then((resp) => {
                resolve(resp.data)
            }).catch(error => {
     ...
     * 注意：后台逻辑是同步的，异步仅通过前端的 Promise 实现
     * @param {string} apiName - Controller/Action 格式的路由
     * @param {object|string} actionPara - 参数对象或 JSON 字符串
...
jQuery.extend(jshelper, JsCrm.common);

jQuery.extend(jshelper, JsCrm.webApi);
